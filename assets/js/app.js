/* Behaviour module.
   ...
   The interaction code shares one closure — DOM refs, the `state` object,
   and helpers such as showToast() and motionReduced(). Threading all of
   that through imports would add indirection for no functional gain, so it
   stays together; what was genuinely separable (the case-study data) is
   what came out.

   Module scope replaces the IIFE, so 'use strict' is implicit. */
...

{
  ...
  // projectData moved to ./project-data.js
